...
void CaCUDA_CopyToDev( CCTK_ARGUMENTS)
{
  ...
  CUDA_SAFE_CALL (cudaMemcpy((void *)d_vx, vx,
          dataSize, cudaMemcpyHostToDevice));

  CUDA_SAFE_CALL (cudaMemcpy((void *)d_vy, vy,
          dataSize, cudaMemcpyHostToDevice));

  CUDA_SAFE_CALL (cudaMemcpy((void *)d_vz, vz,
          dataSize, cudaMemcpyHostToDevice));

  CUDA_SAFE_CALL (cudaMemcpy((void *)d_p, p,
          dataSize, cudaMemcpyHostToDevice));

  CUDA_CHECK_LAST_CALL("Memory copy to device failed");
//  CCTK_INFO("CaCUDA variables vx,vy,vz,p have been successfully copied to device");
}
...
void CaCUDA_CopyFromDev( CCTK_ARGUMENTS)
{
  ...
  CUDA_SAFE_CALL (cudaMemcpy(vx, d_vx,
          dataSize, cudaMemcpyDeviceToHost));

  CUDA_SAFE_CALL (cudaMemcpy(vy, d_vy,
          dataSize, cudaMemcpyDeviceToHost));

  CUDA_SAFE_CALL (cudaMemcpy(vz, d_vz,
          dataSize, cudaMemcpyDeviceToHost));

  CUDA_SAFE_CALL (cudaMemcpy(p, d_p,
          dataSize, cudaMemcpyDeviceToHost));

  CUDA_CHECK_LAST_CALL("Memory copy from device failed");
//  CCTK_INFO("CaCUDA variables vx,vy,vz,p have been successfully copied from device");
}
...
/// The data copy is desigined to minimize the amount of data copied unaligned (E and W ghost-zones).
  ...
  CCTK_REAL * var[]   = {vx, vy, vz, p,  0};
  CCTK_REAL * d_var[] = {d_vx, d_vy, d_vz, d_p,  0};
  char *      names[] = {"vx", "vy", "vz", "p",  0};
